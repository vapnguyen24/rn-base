...
import { API_CONFIG } from '~/src/common/constant';
...
export const handleErrorAxios = <T = Record<string, unknown>>(
  ...
  if (error.code === API_CONFIG.STATUS_TIME_OUT) {
    // timeout
    return handleErrorApi(API_CONFIG.CODE_TIME_OUT) as unknown as ResponseBase<T>;
  }

  if (error.response) {
    if (error.response.status === API_CONFIG.RESULT_CODE_PUSH_OUT) {
      return handleErrorApi(API_CONFIG.RESULT_CODE_PUSH_OUT) as unknown as ResponseBase<T>;
    } else {
      return handleErrorApi(error.response.status) as unknown as ResponseBase<T>;
    }
  }

  return handleErrorApi(API_CONFIG.ERROR_NETWORK_CODE) as unknown as ResponseBase<T>;
};
